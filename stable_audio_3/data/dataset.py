...
class HTSATSampleDataset(torch.utils.data.Dataset):
    def __init__(
        self, 
        configs,
        preprocessor_config,
        ...
        from ..models.htsat import HTSATMelPreprocessor

        self.filenames = []

        self.root_paths = []

        self.sr = preprocessor_config.pop("sample_rate", sample_rate)

        self.custom_metadata_fns = {}

        for config in configs:
            self.root_paths.append(config.path)
            self.filenames.extend(get_audio_filenames(config.path, config.keywords, filelist_path=config.filelist_path))
            if config.custom_metadata_fn is not None:
                self.custom_metadata_fns[config.path] = dill.dumps(config.custom_metadata_fn)

        print(f'Found {len(self.filenames)} files')

        self.preprocessor = HTSATMelPreprocessor(sample_rate=self.sr, **preprocessor_config)
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            mels = self.preprocessor.preprocess_single_to_mels(audio)

            if self.return_mels:
                audio_features = mels
            else:
                audio_features = self.preprocessor.preprocess_mels(mels)
            ...
            return (audio_features, info)
        except Exception as e:
            print(f'Couldn\'t load file {audio_filename}: {e}')
            return self[random.randrange(len(self))]

class HTSATMelsDataset(torch.utils.data.Dataset):
    def __init__(
        self, 
        configs,
        preprocessor_config,
        ...
        for config in configs:
            self.root_paths.append(config.path)
            self.filenames.extend(get_latent_filenames(config.path, config.latent_extension, config.filelist_path))
            if config.custom_metadata_fn is not None:
                self.custom_metadata_fns[config.path] = dill.dumps(config.custom_metadata_fn)
    ...
    def __getitem__(self, idx):
        mel_filename, md_filename = self.filenames[idx]
        try:
            start_time = time.time()
            mels = torch.from_numpy(np.load(mel_filename))
            ...
            for custom_md_path in self.custom_metadata_fns.keys():
                if custom_md_path in mel_filename:
                    custom_metadata_fn = dill.loads(self.custom_metadata_fns[custom_md_path])
            ...
            print(f'Couldn\'t load file {mel_filename}: {e}')
            return self[random.randrange(len(self))]
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True, tokenizers=None, pad=True):
    ...
    if dataset_type == "audio_dir_htsat":
        ...
        htsat_preprocessor_config = dataset_config.get("preprocessor_config", None)
        assert htsat_preprocessor_config is not None, "Must provide HTSAT preprocessor config for dataset_type audio_dir_htsat"

        return_mels = dataset_config.get("return_mels", False)

        train_set = HTSATSampleDataset(
            configs,
            sample_rate=sample_rate,
            preprocessor_config=htsat_preprocessor_config,
            return_mels=return_mels
        )

        if return_mels:
            # Don't batch the data if returning mels, as they will be of different lengths
            collate_fn = lambda x: x
        else:
            collate_fn = collation_fn        

        return torch.utils.data.DataLoader(train_set, batch_size, shuffle=shuffle,
                                num_workers=num_workers, persistent_workers=True, pin_memory=True, drop_last=dataset_config.get("drop_last", True), collate_fn=collate_fn)

    elif dataset_type == "mel_htsat":
        ...
        for audio_dir_config in audio_dir_configs:
            ...
            configs.append(
                LatentDatasetConfig(
                    ...
                    path=audio_dir_path,
                    custom_metadata_fn=custom_metadata_fn,
                    latent_extension=audio_dir_config.get("latent_extension", 'npy'),
                    filelist_path=audio_dir_config.get("filelist_path", None)
                )
            )
