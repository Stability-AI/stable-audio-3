...
instrument: A musical instrument used to produce sound.
...
3. Add supporting elements or layers such as pads, harmonics, effects, vocals, or field recordings.
...
- Cinematic orchestral adventure with full strings, brass ensemble, timpani, snare rolls, harp glissandos, and choir pads building epic heroic tension and grandeur. BPM: 95. Length: 300 seconds
...
- Upbeat pop track with acoustic guitar, electric piano, synth bass, bright drum kit, layered vocal harmonies, and brass stabs that feel cheerful and danceable. BPM: 120. Length: 180 seconds
...
- Orchestral grandeur with violins, violas, cellos, double bass, French horns, trumpets, timpani, harp, cymbals, and choir pads evoking majestic heroic emotion. BPM: 90. Length: 300 seconds
...
- 90s pop with bright synths, electric guitar rhythm, bass guitar, electronic drum kit, layered backing vocals, and soft piano accents for catchy, upbeat fun. BPM: 118. Length: 180 seconds
...
- Folk storytelling song with acoustic guitar, fiddle, upright bass, soft percussion with tambourine and brushes, layered vocal harmonies, and subtle organ pads for earthy warm emotion. BPM: 95. Length: 200 seconds
- Hybrid classical-electronic track with piano lead, strings, synth textures, arpeggiated pads, electronic percussion, glitch effects, and choir pads creating dramatic futuristic intensity. BPM: 85. Length: 250 seconds
...
- Country ballad with acoustic guitar, fiddle, upright bass, gentle drums, subtle organ pads, and layered vocal harmonies for heartfelt warm emotion. BPM: 80. Length: 180 seconds
...
- K-pop upbeat track with bright synths, electric guitar, bass, layered vocals, snappy electronic drums, and sparkling pads creating energetic catchy hooks. BPM: 130. Length: 180 seconds
...
- Trap banger with 808 bass, hi-hat rolls, snare, layered synth leads, vocal chops, and atmospheric pads for dark high-energy club feel. BPM: 140. Length: 150 seconds
- Synth-pop dreamy track with arpeggiated synths, analog pads, soft drums, electronic bass, and lush vocal layers for nostalgic melodic energy. BPM: 115. Length: 180 seconds
...
- Cinematic romance orchestral track with strings, piano, harp, soft choir, timpani, and woodwinds for emotional sweeping grandeur. BPM: 70. Length: 300 seconds
- Chillstep track with lush pads, soft piano leads, melodic synths, chilled percussion, deep sub-bass, and vocal textures for relaxing atmospheric vibes. BPM: 90. Length: 200 seconds
- Electro swing track with brass leads, upright bass, piano, vintage drums, layered vocals, and swing-style percussions creating playful jazzy dance energy. BPM: 125. Length: 180 seconds
...
- Upbeat 1950s rock and roll with driving boogie-woogie piano, walking bass, handclaps, energetic male vocals, and vintage analog recording texture for a lively and raw feel. BPM: 168. Length: 180 seconds
...
- A piece of ethereal, dreamy and beautiful music perfect for a surreal 80s film score, featuring soft modulating synthesizer pads, ambient sfx and foley, atmospheric flute, and beautiful choirs. The mood is cinematic, retro, vintage and nostalgic. BPM: 60. Length: 300 seconds""",
...
- Old radio static with faint distorted voices, unsettling tone. Length: 30 seconds
    ...
    if tags_mode:
        ...
        for tag in source.split(","):
            ...
            if tag and tag not in seen:
                seen.add(tag)
                deduped.append(tag)
        input_lower = prompt.strip().lower()
        if input_lower not in seen:
            deduped.insert(0, input_lower)
        result = ", ".join(deduped)
    else:
        result = source.strip()
...
def reprompt(prompt, preset, system_prompt, model_id, max_new_tokens, temperature):
    if not prompt.strip():
        ...
        return example, example, None

    tokenizer, model = get_model(model_id)

    category = None
    if preset == "Auto":
        label = _run_model(tokenizer, model, SYSTEM_PROMPTS["Classifier"], prompt, 16, 1.0)
