...
DURATION_SEC = 2  # seconds — keep short for speed
STEPS = 8
...
def test_text_to_audio(model_pipe, maybe_save_audio):
    ...
    prompt = "saxophone solo"

    audio = pipe.generate(
        prompt=prompt,
        negative_prompt="loud background noise",
...
def test_outpainting(model_pipe, maybe_save_audio):
    pipe = model_pipe
    sr = pipe.model_config["sample_rate"]
    channels = pipe.model_config.get("io_channels", 2)
    init_duration = 5.0
...
def test_batch_inference(model_pipe, maybe_save_audio):
    ...
    for i in range(batch_size):
        for j in range(i + 1, batch_size):
            diff = torch.mean(torch.abs(audio_same_durations[i] - audio_same_durations[j])).item()
            diffs.append(diff)
    ...
    for i, dur in enumerate(durations):
        ...
        maybe_save_audio(audio_different_durations[i : i + 1], sr, f"{prompts[i]}_{dur}s")
